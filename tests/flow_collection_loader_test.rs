...
#[cfg(feature = "source_mapping")]
use yaml_rust2::source_map::{NodeId, SourceMap, SourceMapSupport};
...
fn print_position_span(span: &PositionSpan, label: &str) {
    println!("{} start: ({}, {})", label, span.start.line(), span.start.col());
...
"#;
    ...
    parser.load(&mut loader, true).expect("Failed to parse YAML");
    ...
    let root_flow = find_node_by_path(document, &["root_flow"])
        .expect("Should find root_flow node");
    ...
    if let Yaml::Hash(hash) = root_flow {
        assert!(hash.contains_key(&Yaml::String("key1".to_string())), "Should contain key1");
        assert!(hash.contains_key(&Yaml::String("key2".to_string())), "Should contain key2");
        ...
        if let Some(location) = source_map.get_location(root_flow_id) {
            ...
            assert!(location.span.end.is_some(), "Root flow mapping should have an end position");
            
            // Verify we can find the node at its position
            let found_node = source_map.find_node_at_position(location.start_line(), location.start_column());
    ...
    // Find the node ID by using pointer equality
    let (nested_flow_id, _) = find_node_by_path_with_id(document, source_map, &["nested", "flow_map"])
        .expect("Should find nested_flow node ID");
    
    if let Yaml::Hash(hash) = nested_flow {
        assert!(hash.contains_key(&Yaml::String("inner1".to_string())), "Should contain inner1");
        assert!(hash.contains_key(&Yaml::String("inner2".to_string())), "Should contain inner2");
        ...
        if let Some(location) = source_map.get_location(nested_flow_id) {
            ...
            // Verify end position exists
            assert!(location.span.end.is_some(), "Nested flow mapping should have an end position");
    ...
    let root_seq = find_node_by_path(document, &["root_seq"])
        .expect("Should find root_seq node");
    ...
    if let Yaml::Array(array) = root_seq {
        ...
        if let Some(location) = source_map.get_location(root_seq_id) {
            ...
            // Verify end position exists
            assert!(location.span.end.is_some(), "Root flow sequence should have an end position");
    ...
    let complex_path = find_node_by_path(document, &["complex", "outer_key", "inner_key", "2", "deep_key"])
        .expect("Should find complex.outer_key.inner_key[2].deep_key");
    
    assert_eq!(complex_path, &Yaml::String("deep_value".to_string()), "Should have correct deep value");
    ...
    if let Some(location) = source_map.get_location(complex_id) {
        ...
        // Verify end position exists
        assert!(location.span.end.is_some(), "Complex mapping should have an end position");
