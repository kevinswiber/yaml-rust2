...
/// YAML events that can be produced by the parser.
#[derive(Debug, PartialEq, Clone)]
pub enum Event {
    /// Nothing, a marker for initialization
    Nothing,
...
pub struct Parser<T> {
    ...
    ///
    /// This is used to track the positions of opening and closing
    /// delimiters for flow mappings and sequences.
...
pub trait MarkedEventReceiver {
    ...
    /// Handler called for each event that occurs with full position span information.
    ///
    /// This method provides both start and end positions for YAML constructs
    /// that span multiple tokens. This is particularly useful for flow mappings
    /// and sequences, where the start and end positions should point to the
    /// opening and closing delimiters.
    ///
    /// The default implementation calls the original `on_event` method with
    /// just the start position for backward compatibility.
...
impl<T: Iterator<Item = char>> Parser<T> {
    ...
    /// Load the YAML from the stream in `self`, pushing events with accurate position tracking into `recv`.
    ///
    /// This method is similar to `load`, but enhances events with accurate position spans,
    /// particularly for flow collections. This enables better source mapping in tools that
    /// use the YAML parser.
    ///
    /// If `multi` is set to `true`, the parser will allow parsing of multiple YAML documents
    /// inside the stream.
    ...
    /// # Errors
    /// Returns `ScanError` when loading fails.
    pub fn load_with_positions<R: MarkedEventReceiver>(
        &mut self,
        recv: &mut R,
        multi: bool,
    ) -> Result<(), ScanError> {
    ...
    fn load_document_with_positions<R: MarkedEventReceiver>(
        &mut self,
        first_ev: Event,
        mark: Marker,
        recv: &mut R,
        ...
        // Use position tracking for the event
        let span = self.get_position_span(&first_ev, mark);
        recv.on_positioned_event(first_ev, span);
    ...
    fn load_node_with_positions<R: MarkedEventReceiver>(
        &mut self,
        first_ev: Event,
        mark: Marker,
        recv: &mut R,
        ...
        match first_ev {
            ...
            Event::SequenceStart(..) => {
                ...
                let span = self.get_position_span(&first_ev, mark);
                recv.on_positioned_event(first_ev, span);
                self.load_sequence_with_positions(recv)
            }
            Event::MappingStart(..) => {
                // Use position tracking for the event
                let span = self.get_position_span(&first_ev, mark);
                recv.on_positioned_event(first_ev, span);
                self.load_mapping_with_positions(recv)
            }
            _ => {
                println!("UNREACHABLE EVENT: {first_ev:?}");
                unreachable!();
    ...
    fn load_mapping_with_positions<R: MarkedEventReceiver>(
        &mut self,
        recv: &mut R,
    ) -> Result<(), ScanError> {
        loop {
    ...
    fn load_sequence_with_positions<R: MarkedEventReceiver>(
        &mut self,
        recv: &mut R,
    ) -> Result<(), ScanError> {
        loop {
    ...
    /// Get the position span for the current event
    ///
    /// This method uses the parser's position tracker to get the position span
    /// for the given event and mark. This is useful for getting the complete
    /// span for flow collections, where the span should include both the opening
    /// and closing delimiters.
    pub fn get_position_span(
        &mut self,
        event: &Event,
        mark: Marker,
